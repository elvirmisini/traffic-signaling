...
def swap_random_orders(current_solution: list[Schedule], nr_changes) -> list[Schedule]:
    tweaked_solution = deepcopy(current_solution)

    num_to_swap = max(1, len(tweaked_solution) * nr_changes // 100)
    for _ in range(num_to_swap):
        schedule = random.choice(tweaked_solution)
        if len(schedule.order) > 1:
            index1, index2 = random.sample(range(len(schedule.order)), 2)
...
def optimize_orders_brute_force(current_solution: list[Schedule],
    ...
    # Select a random subset of schedules to optimize
    percentage_to_optimize = 0.0015  # 0.15% as a fraction
    num_to_optimize = max(1, int(len(current_solution) * percentage_to_optimize))
    schedules_to_optimize = random.sample(current_solution, num_to_optimize)

    # Iterate through selected schedules
    for schedule in schedules_to_optimize:
        original_order = schedule.order
        n = len(original_order)

        # Skip schedules with fewer than 3 phases
        if n < 3:
            continue

        # Precompute slices of original order to reduce redundant calculations
        for i in range(n - 2):
            elements_to_permute = original_order[i:i + 3]
            original_prefix = original_order[:i]
            original_suffix = original_order[i + 3:]

            # Evaluate all permutations
            for permuted in itertools.permutations(elements_to_permute):
                schedule.order = original_prefix + list(permuted) + original_suffix
                temp_score = fitness_score(current_solution, streets, intersections, paths, total_duration,
                                           bonus_points)

                # Update the best solution if a better score is found
...
def perturb(current_solution: list[Schedule]) -> list[Schedule]:
    ...
    if perturb_option == 1:
        return swap_random_orders(current_solution, 10)
    else:
...
def optimize_solution_with_ils(initial_solution: list[Schedule],
    ...
    # Track the time  the best solution is found
    ...
    while time.time() - start_time < duration:
        ...
        selected_inner_iteration = random.choice([50, 100, 200, 500, 1000])
